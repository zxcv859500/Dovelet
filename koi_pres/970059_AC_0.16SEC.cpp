...
void result(int cnt){
    ...
    else if(cnt==3) printf(" %d",th_cnt);
    if(cnt==0){
		if(fi_cnt>se_cnt&&fi_cnt>se_cnt) printf(" %d",fi_cnt);
		else if(se_cnt>fi_cnt&&se_cnt>th_cnt) printf(" %d",se_cnt);
...
int main(void){
    ...
    else if(fi_cnt==se_cnt||fi_cnt==th_cnt||se_cnt==th_cnt){
